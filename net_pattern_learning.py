...
y_ = model.my_net(x_image,use_bn = False)
...
#the layers that need to trained
...
with tf.name_scope( 'train_op' ): # tf.name_scope() 这个方法的作用不太明白(有点迷糊!)
    ...
    #定义优化算子
    optimizer = tf.train.AdamOptimizer(1e-5,beta1=0.9,beta2=0.999,name="Adam")
    #选择待优化的参数
    #for i in train_layer:
    output_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="")
    train_op = optimizer.minimize(loss,var_list = output_vars)
...
freeze_conv_var_list = [t for t in trainable_vars]
for i in freeze_conv_var_list:
    print(i,'\n')
...
with tf.Session() as sess:
    ...
    for i in range( train_steps ):
        ...
        if ( i+1 ) % 50 == 0:
            ...
            print('----------------------------------------------------')
        if(i+1)%10000==0:
            print("the iterator of real time is ",count)
            save_path = saver.save(sess, model_save_path)
            print("Model saved in file: %s" % save_path)
    
plt.figure()
plt.plot(list_count,loss_val_all)
plt.savefig("/home/zhengmiao/cifar10/output_image/leng_1_count_100000/all_normal.png")

plt.figure()
plt.plot(list_count,acc_val_all)
plt.savefig("/home/zhengmiao/cifar10/output_image/leng_1_count_100000/all_normal_acc.png")
